...
将一个轻量级分布式计算框架[Rain](https://github.com/substantic/rain)部署在树莓派上，实现以树莓派为代表的IoT设备的算力利用。
...
Iot设备数量虽然庞大，但由于种类繁多、应用环境复杂、单个设备性能较低等特点，尝试使用其多余算力并不具备很强的意义。本次大作业以树莓派为代表，通过在其上尝试部署分布式运算节点并运行，仍具有一定的学习意义。
